...
sudo systemctl enable $SYSD_NAME.service
sudo systemctl start $SYSD_NAME.service

# add module to /etc/modules for auto load
echo "Add module to /etc/modules.d for auto load"
sudo cp ./service/kb-ench.conf /etc/modules-load.d/
...
if [ -z "$output" ]; then
    ...
    sudo systemctl status systemd-modules-load.service | grep "active (.*)" || { echo "Error: failed to start systemd-modules-load.service"; exit 1; }
fi

echo "Create themes folder"

if ! [[ -d $HOME/.config/kb-ench/themes ]]; then
    mkdir -p $HOME/.config/kb-ench/themes
fi

echo "Copy default theme"

cp -r ./themes/rose-pine.css $HOME/.config/kb-ench/theme.css
